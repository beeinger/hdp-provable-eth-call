...
use crate::utils::eth_transaction::common::{TxKind, TxKindTrait};
use crate::utils::eth_transaction::eip1559::{TxEip1559, TxEip1559Trait};
use crate::utils::eth_transaction::eip2930::{AccessListItem, TxEip2930, TxEip2930Trait};
use crate::utils::eth_transaction::legacy::TxLegacy;
use crate::utils::eth_transaction::tx_type::TxType;
use crate::utils::rlp::{RLPHelpersTrait, RLPItem, RLPTrait};
use crate::utils::traits::DefaultSignature;
...
pub impl _TransactionUnsigned of TransactionUnsignedTrait {
    ...
    fn decode_enveloped(ref tx_data: Span<u8>) -> Result<TransactionUnsigned, EthTransactionError> {
        if tx_data.is_empty() {
    ...
    fn decode_legacy_tx(
        ...
        let rlp_decoded_data: Span<RLPItem> = RLPErrorTrait::map_err(rlp_decoded_data)?;
        ...
        let legacy_tx: TxLegacy = match rlp_decoded_data {
            ...
            RLPItem::List(mut val) => {
                ...
                let nonce = RLPErrorTrait::map_err(nonce_encoded.parse_u64_from_string())?;
                let gas_price = RLPErrorTrait::map_err(gas_price_encoded.parse_u128_from_string())?;
                let gas_limit = RLPErrorTrait::map_err(gas_limit_encoded.parse_u64_from_string())?;
                let to = RLPErrorTrait::map_err(to_encoded.try_parse_address_from_string())?;
                let value = RLPErrorTrait::map_err(value_encoded.parse_u256_from_string())?;
                let input = RLPErrorTrait::map_err(input_encoded.parse_bytes_from_string())?;
                let chain_id = RLPErrorTrait::map_err(chain_id_encoded.parse_u64_from_string())?;
    ...
    fn decode_enveloped_typed_transaction(
        ...
        let rlp_decoded_data = RLPErrorTrait::map_err(RLPTrait::decode(encoded_tx_data))?;
        if (rlp_decoded_data.len() != 1) {
            return Result::Err(
...
mod tests {
    ...
    use crate::utils::test_data::{
        eip_1559_encoded_tx, eip_2930_encoded_tx, legacy_rlp_encoded_deploy_tx,
