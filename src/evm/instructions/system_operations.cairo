use crate::evm::call_helpers::CallHelpers;
use crate::evm::create_helpers::{CreateHelpers, CreateType};
use crate::evm::errors::{EVMError, ensure};
use crate::evm::gas;
use crate::evm::memory::MemoryTrait;
use crate::evm::model::Transfer;
...
mod tests {
    ...
    use crate::utils::contracts_test_data::{storage_evm_bytecode, storage_evm_initcode};
    use crate::utils::helpers::{compute_starknet_address, load_word};
    ...
    fn test_exec_call() {
        ...
        let bytecode = [
            0x60, 0x01, 0x60, 0x00, 0x60, 0x00, 0x60, 0x00, 0x60, 0x00, 0x64, 0xab, 0xfa, 0x74,
            0x0c, 0xcd, 0x62, 0xff, 0xff, 0xff, // CALL
            0xf1, 0x00,
        ]
            .span();
    ...
    fn test_should_fail_exec_staticcall() {
        ...
        let bytecode = [
            0x60, 0x01, 0x60, 0x00, 0x60, 0x00, 0x60, 0x00, 0x60, 0x00, 0x64, 0xab, 0xfa, 0x74,
            0x0c, 0xcd, 0x62, 0xff, 0xff, 0xff, // STATICCALL
            0xfa, 0x00,
        ]
            .span();
        let code_hash = bytecode.compute_keccak256_hash();
    ...
    fn test_exec_call_code() {
        ...
        let bytecode = [
            0x60, 0x01, 0x60, 0x00, 0x60, 0x00, 0x60, 0x00, 0x60, 0x00, 0x60, 0x00, 0x61, 0x12,
            0x34, 0x62, 0xff, 0xff, 0xff, // CALLCODE
            0xf2, 0x00,
        ]
            .span();
    ...
    fn test_exec_delegatecall() {
        ...
        let bytecode = [
            0x60, 0x01, 0x60, 0x00, 0x60, 0x00, 0x60, 0x00, 0x60, 0x00, 0x61, 0x12, 0x34, 0x62,
            0xff, 0xff, 0xff, // DELEGATECALL
            0xf4, 0x00,
        ]
            .span();
        let _code_hash = bytecode.compute_keccak256_hash();
