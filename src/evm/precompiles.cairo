    ...
    fn exec_precompile(ref vm: VM) -> Result<(), EVMError> {
        ...
        let (gas, result) = if precompile_address.into() == 0x100 {
            ...
            match precompile_address.into() {
                ...
                0x06 => {
                    panic!("pre-compile at address {:?} is not working yet", precompile_address)
                },
                0x07 => {
                    panic!("pre-compile at address {:?} is not working yet", precompile_address)
                },
...
mod tests {
    ...
    fn test_eth_precompile_addresses() {
        ...
        assert_eq!(
            addresses.to_span(),
            [
                0x01.try_into().unwrap(), 0x02.try_into().unwrap(), 0x03.try_into().unwrap(),
                0x04.try_into().unwrap(), 0x05.try_into().unwrap(), 0x06.try_into().unwrap(),
                0x07.try_into().unwrap(), 0x08.try_into().unwrap(), 0x09.try_into().unwrap(),
